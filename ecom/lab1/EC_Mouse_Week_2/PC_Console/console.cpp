...
void ReadSPI(char spi_cmd, char spi_value)
{
	...
	char spi_cmd_send = ((spi_value<<7)||spi_cmd);
	
    #define MYCOMMAND    1
    send_buf[0] = MYCOMMAND;      // Command
    send_buf[1] = 0x01;              // Expected length of the result
	send_buf[2] = spi_cmd_send;

    if(SendReceivePacket(send_buf,2,receive_buf,&RecvLength,1000,1000) == 1)
    {
        if(RecvLength == 4 && receive_buf[0] == MYCOMMAND &&
            receive_buf[1] == 0x01)
        {
            if(receive_buf[2]==1 && receive_buf[3]==1) printf("\nS2 and S3 pressed\r\n");
            else if(receive_buf[2]==1 && receive_buf[3]==0) printf("\nS2 pressed\r\n");
