...
void DrawCapturedImage(void)
{
    ...
    send_buf[0] = 1;      // CommWriteWithSPIHalfDuplex(char address, char data)and
    send_buf[1] = 0x02;            // Expected length of the result

    if(SendReceivePacket(send_buf,2,receive_buf,&RecvLength,1000,1000) == 1)
    {
        if(RecvLength == 2 && receive_buf[0] == 1 && receive_buf[1] == 0x02)
        {
    ...
    send_buf[0] = 0;      // CommWriteWithSPIHalfDuplex(char address, char data)and
    send_buf[1] = 0x40;            // Expected length of the result
    ...
    for(int i = 0; i < 4; i++)
    {
        if(SendReceivePacket(send_buf,2,receive_buf,&RecvLength,1000,1000) == 1)
        {
            if(RecvLength == 64 && receive_buf[0] == 0 && receive_buf[1] == 0x40)
            {
                for(int j=2; j<64; j++)
                {
                    if(aux<225)
                    {
                        image[aux]=receive_buf[j];
                        aux++;
                    }
                }
            }
